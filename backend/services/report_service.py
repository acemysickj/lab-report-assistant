...
async def generate_with_review(
    ...
    # --- Step 3: Stream blocks JSON to frontend ---
    result_json = json.dumps({"blocks": blocks}, ensure_ascii=False)
    chunk_size = 300
    for i in range(0, len(result_json), chunk_size):
        yield {"type": "chunk", "content": result_json[i:i + chunk_size]}
...
async def _revise_content(
    ...
    system_prompt = (
        ...
        "【输出铁律】\n"
        "1. 只输出 JSON 数组（blocks），不要输出任何其他内容\n"
        "2. 每个元素格式：{\"type\":\"body\",\"text\":\"段落内容\"} 或 "
        "{\"type\":\"section_heading\",\"text\":\"标题\"} 或 "
        "{\"type\":\"display_formula\",\"latex\":\"公式\"} 或 "
        "{\"type\":\"three_line_table\",\"headers\":[...],\"rows\":[[...]],\"caption\":\"...\"}\n"
        "3. 内联公式用 $...$ 包裹，如 $E=mc^2$\n"
        "4. 禁止输出修改说明、审阅意见、思考过程\n"
        "5. 禁止用 ```json 代码块包裹（直接输出裸 JSON 数组）\n"
        "6. 禁止输出 HTML 标签（<h2>、<div>、<p> 等），只使用上述 JSON 格式"
    )
