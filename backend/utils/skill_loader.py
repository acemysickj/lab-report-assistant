...
def build_system_prompt(
    ...
    prompt_parts = [
        ...
        "  - 不需要编号的公式用 $$...$$",
        "- 输出应为 HTML 片段（不含 <!DOCTYPE>, <html>, <head>, <body> 标签）",
        "- 禁止在片段内写 <style> 标签，全局样式由组装模板统一提供（包括三线表格式）",
        "- 表格使用标准 <table> 标签即可，模板会自动渲染为三线表",
        "",
        "【实验步骤特殊规范 — 架构图风格】",
        ...
        "",
        "【输出铁律 — 违反即为不合格】",
        "- 直接输出 HTML 内容，禁止任何引导语、说明、总结",
        "- 禁止输出「好的」「以下是」「修改说明」等非报告内容",
        "- 禁止用代码块（```html ... ```）包裹",
        "- 你的输出就是最终报告，用户只看到内容本身",
    ]
    ...
    prompt_parts.extend([
        "",
        f"请为「{exp_title}」生成【{section}】部分的 HTML 内容。",
    ])
...
def build_review_prompt(
    ...
    prompt_parts = [
        ...
        "5. LaTeX 公式检查：",
        "   - 所有公式必须用 $$...$$ 或 \\(...\\) 包裹",
        "   - 编号公式必须用 \\begin{equation}...\\end{equation}，禁止用 \\tag{}",
        "   - 不得出现裸写的 LaTeX 命令（\\frac、\\sum、\\Delta 等）",
        "   - 大括号必须配对，语法必须正确",
