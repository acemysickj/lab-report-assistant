...
def proc_table(g):
    ...
    for row_i in range(2):
        ...
        label = "t/min（续表）" if row_i > 0 else "t/min"
        parts.append(f'<table class="data"><thead><tr><th>{label}</th>{cols}</tr></thead><tbody><tr><td>at / deg</td>{ats}</tr><tr><td>ln(at - ainf)</td>{lns}</tr></tbody></table>')
    return "".join(parts)
...
<style>
  ...
  table.data {{ width: 100%; border-collapse: collapse; margin: 0 auto 6px; font-size: 13px; }}
  table.data thead {{ border-top: 1.5pt solid #000; border-bottom: 0.5pt solid #000; }}
  table.data tbody {{ border-bottom: 1.5pt solid #000; }}
